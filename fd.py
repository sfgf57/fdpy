    ...
    with open(CACHE_FILE, "r", encoding="utf-8") as f:
        ...
        for line in reversed(lines):
            try:
                ...
                if any(word.lower() in caption.lower() for word in user_query.split()):
...
def format_small_text(text: str) -> str:
    """Convert normal text to small superscript/subscript style"""
    small_map = {
        'a': 'ᵃ', 'b': 'ᵇ', 'c': 'ᶜ', 'd': 'ᵈ', 'e': 'ᵉ', 
        'f': 'ᶠ', 'g': 'ᵍ', 'h': 'ʰ', 'i': 'ⁱ', 'j': 'ʲ',
        'k': 'ᵏ', 'l': 'ˡ', 'm': 'ᵐ', 'n': 'ⁿ', 'o': 'ᵒ',
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🔍 🔍 Send me a movie or web series name, and I’ll find related videos for you!! || मुझे कोई फ़िल्म या वेब सीरीज़ का नाम भेजो, मैं उससे जुड़े वीडियो ढूंढकर भेजूंगा!    Example :- panchayat  , squid game ")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not matches:
        await update.message.reply_text("❌ No matching content found. Please check your spelling or try searching correct spelling on Google. If the spelling is correct and content is still not found, 🖍️ report it to @moviezonehelp with the correct spelling. Your content may become available within a few days, so feel free to request again later. || ❌ कोई मेल खाता कंटेंट नहीं मिला। कृपया अपनी वर्तनी जांचें या Google पर खोजें। अगर वर्तनी सही है और फिर भी कंटेंट नहीं मिला, तो 🖍️ कृपया सही वर्तनी के साथ @moviezonehelp पर रिपोर्ट करें। आपका कंटेंट कुछ दिनों में उपलब्ध हो सकता है, इसलिए बाद में दोबारा अनुरोध करें। ")
        return
